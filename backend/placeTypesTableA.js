// ---------------------------------------------------------------------------
// Google Places API (New) — "Table A" place types, i.e. the types that can
// be used to FILTER a search (via includedType), as opposed to "Table B"
// types which are response-only and can't be searched by. Source:
// https://developers.google.com/maps/documentation/places/web-service/place-types
// (fetched while building this dashboard — re-check that page periodically,
// Google does add new types over time).
//
// This is deliberately exhaustive per the build brief ("EVERY Table A place
// type"), not curated down to Moroccan-SME-relevant ones like the CLI's
// config.js SECTORS list — some categories below (Geographical Areas,
// Natural Features, Transportation infrastructure, Places of Worship) are
// not typical prospecting sectors but are included for completeness, with
// the search/filter box in the UI expected to be how users actually narrow
...
const RAW_CATEGORIES = {
  ...
  'Entertainment and Recreation': [
    ...
    'dog_park', 'event_venue', 'ferris_wheel', 'garden', 'go_karting_venue', 'hiking_area',
    ...
    'paintball_center', 'park', 'philharmonic_hall', 'picnic_ground', 'planetarium', 'plaza',
...
const CATEGORY_LABELS_FR = {
  ...
  Business: 'Entreprises & Bureaux',
  Culture: 'Culture',
  Education: 'Éducation',
  'Entertainment and Recreation': 'Loisirs & Divertissement',
  Facilities: 'Équipements publics',
  Finance: 'Finance',
  'Food and Drink': 'Restauration & Boissons',
  'Geographical Areas': 'Zones géographiques',
  Government: 'Administration publique',
  'Health and Wellness': 'Santé & Bien-être',
  Housing: 'Logement',
  Lodging: 'Hébergement',
  'Natural Features': 'Sites naturels',
...
// Fallback label for types not in config.js's curated French TYPE_LABELS
// dictionary (most of these ~350 types aren't, since that dictionary only
// covers the CLI's curated SME sectors): "hardware_store" -> "Hardware Store".
// Not hand-translated to French — see README "Assumptions" for why.
